...
class HomeViewController: UIViewController, CLLocationManagerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITableViewDragDelegate, UITableViewDropDelegate, UNUserNotificationCenterDelegate {
    ...
    //MARK: - ALL NOTES LOGIC
        ...
        func saveNote() {
            ...
            guard let activeCell = activeNoteCell,
            ...
            getLocationName(from: locationToSave) { [weak self] locationName in
                ...
                print("Debug: Determined location name to save as '\(locationNameToSave)'.")
                ...
                let noteData: [String: Any] = [
                    "user": userEmail,
                    "note": noteText,
                    "location": GeoPoint(latitude: locationToSave.latitude, longitude: locationToSave.longitude),
                ...
                self.db.collection("notes").document(noteId).setData(noteData) { error in
                    if let error = error {
                        ...
                        print("Note successfully saved to Firestore with location name: '\(locationNameToSave)'.")
                        if let noteIndex = self.notes.firstIndex(where: { $0.id == noteId }) {
                            ...
                            DispatchQueue.main.async {
                                ...
                                self.updateUI(withLocationName: locationNameToSave) // Ensure this is using the same location name
                            }
                        }
        ...
        func determineLocationName(userChosenName: String?, autoGeneratedName: String?) -> String {
            if let userChosenName = userChosenName, !userChosenName.isEmpty {
                print("Debug: Using user-chosen location name: '\(userChosenName)'.")
                return userChosenName
            } else if let autoGeneratedName = autoGeneratedName, !autoGeneratedName.isEmpty {
                print("Debug: Using auto-generated location name: '\(autoGeneratedName)'.")
                return autoGeneratedName
            } else {
                print("Debug: Defaulting to 'Unnamed Location' as no name was provided or determined.")
    ...
    //                            self?.updateProgressBar()
        ...
        func createNewNoteWithTranscription(_ transcription: String) {
            // Ensure that you are using the location selected from PlacesViewController
            if let currentLocationCoordinate = self.selectedLocation {
                let userDefinedLocationName = self.currentLocationName ?? "Unknown Location"
                let emptyURL = URL(string: "")  // Assuming this is a placeholder for something relevant
                ...
                selectedNote = newNote  // Assuming this sets the context for something relevant
                ...
                print("No selected location to attach to the transcription.")
                // Consider showing an alert or another form of feedback to the user
            }
        }
        
        //Save Name Button
        @IBAction func SaveNote(_ sender: UIButton) {
            // Animation to shrink the button
                UIView.animate(withDuration: 0.2, animations: {
                    sender.transform = CGAffineTransform(scaleX: 0.90, y: 0.90)
                }) { _ in
                    // Animation to return to the original size
        ...
        @IBAction func NewNote(_ sender: UIButton) {
            ...
            // Animation to shrink the button
                ...
                // Use a default empty string or predefined text for new notes
                let defaultNoteText = "" // You can change this to any default text you want for new notes
                
                // Use the current displayed location name and its coordinates
                let userDefinedLocationName = currentLocationName ?? ""
                
                // Determine which location to use: selectedLocation or currentLocation
                if let currentLocationCoordinate = self.selectedLocation ?? self.currentLocation {
                    let emptyURL = URL(string: "")
                    let newNote = Note(id: UUID().uuidString, text: defaultNoteText, location: currentLocationCoordinate, locationName: userDefinedLocationName, imageURL: emptyURL)
                    
                    // Append the new note to the notes array and select it
                    notes.append(newNote)
                    ...
                    DispatchQueue.main.async {
                        ...
                        // Automatically focus on the new note's text field after a short delay
                    ...
                    print("Failed to get user's current location or selected location for new note")
                }
            }
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        reverseGeocode(location: newLocation.coordinate) { [weak self] locationName in
            DispatchQueue.main.async {
                // Update UI with new location name
                self?.currentLocationName = locationName ?? "Unknown Location"
                self?.updateLocationNameLabel(location: self?.currentLocation ?? newLocation.coordinate)
                self?.displayImage(location: self?.currentLocation ?? newLocation.coordinate)
                
                if let coord = self?.currentLocation {
                    let locationObj = CLLocation(latitude: coord.latitude, longitude: coord.longitude)
                    self?.loadAndFilterNotes(for: coord, goalRadius: 15.0) {
    ...
    func setupClosestFifteenGeofences(currentLocation: CLLocation, completion: @escaping () -> Void) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 7) {
            print("Setting up geofences based on proximity to the current location.")
            
            // Assuming self.notes is already populated with all available notes:
            ...
            // Consider the top 20 closest notes due to iOS geofence limitations
            let closestNotes = Array(sortedNotes.prefix(20))
            print("Closest notes count for geofencing: \(closestNotes.count)")
            
            // Clear existing geofences to reset them based on new logic
            ...
            for note in closestNotes {
                ...
                let geofence = Geofence(location: coordinate, radius: 100, identifier: note.locationName)  // Adjust radius as needed
            ...
            print("Geofences updated based on current proximity and notes.")
            completion()
        }
    }
    ...
    func locationManager(_ manager: CLLocationManager, didEnterRegion region: CLRegion) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 5) {
            ...
            // Load places notes for the region
            self.LoadPlacesNotes(for: region.identifier) {
                ...
                if !lastThreeNotesText.isEmpty {
                    ...
                    // Ensure image is displayed when entering region
                    self.displayImage(locationName: region.identifier)
                    print("Displaying image for location: \(region.identifier)")
                } else {
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        ...
        if let locationName = userInfo["locationName"] as? String {
            DispatchQueue.main.async { [weak self] in
                // Ensure the app navigates to the HomeViewController
                self?.tabBarController?.selectedIndex = 0  // Set to the index of HomeViewController in your TabBarController
                
                // Obtain HomeViewController and call LoadPlacesNotes
                if let navigationController = self?.tabBarController?.selectedViewController as? UINavigationController,
                   let homeVC = navigationController.viewControllers.first(where: { $0 is HomeViewController }) as? HomeViewController {
                    homeVC.LoadPlacesNotes(for: locationName) {
                        // Scroll to the top of the notes list if there are notes
                        if !homeVC.notes.isEmpty {
                            homeVC.tableView.scrollToRow(at: IndexPath(row: 0, section: 0), at: .top, animated: true)
                        }
                        print("Notification led to HomeViewController")
                    }
                } else if let homeVC = self?.tabBarController?.selectedViewController as? HomeViewController {
                    // Direct instance of HomeViewController without a navigation controller
                    homeVC.LoadPlacesNotes(for: locationName) {
                        if !homeVC.notes.isEmpty {
                            homeVC.tableView.scrollToRow(at: IndexPath(row: 0, section: 0), at: .top, animated: true)
                        }
                        print("Notification led directly to HomeViewController")
                    }
                }
            }
        }
    ...
    var isLocationNameManuallySet = false
    ...
    @IBAction func uploadImageButton(_ sender: UIButton) {
        ...
        shakeAnimation.duration = 0.5 // Duration of the shake
        shakeAnimation.repeatCount = 1 // Number of repeats
        shakeAnimation.autoreverses = false // Do not auto-reverse animation
        
        let fromPoint = CGPoint(x: sender.center.x - 5, y: sender.center.y)
        let toPoint = CGPoint(x: sender.center.x + 5, y: sender.center.y)
        let fromValue = NSValue(cgPoint: fromPoint)
        let toValue = NSValue(cgPoint: toPoint)
        
        shakeAnimation.values = [fromValue, toValue, fromValue, toValue, fromValue] // Move from left to right, and back
        shakeAnimation.keyTimes = [0, 0.25, 0.5, 0.75, 1] // Corresponding times for the keyframes
        
        // Adding some damping to make it look more natural
    ...
    @IBAction func LocationButton(_ sender: UIButton) {
            ...
            UIView.animate(withDuration: 0.2, animations: {
                ...
                // Fetch the name for the current location
                let locationName = self.fetchLocationNameFor(location: userLocation) ?? "Current Location"
                DispatchQueue.main.async {
                    // Set the current location name and update UI
                    self.currentLocationName = locationName
                    self.locationNameLabel.text = locationName
                    print("Location name is \(locationName)")
                    
                    // Set this location as the selected location
                ...
                override func viewDidLoad() {
                    ...
                    locationManager.startUpdatingLocation()  // Start updating location immediately
                    // trigger the location button functionality
                    ...
                    // Set selected location to current location when view first loads
                    if let currentLocation = locationManager.location?.coordinate {
                        selectedLocation = currentLocation  // Assuming 'selectedLocation' is your variable to keep track of this
                        print("Selected location set to current location on viewDidLoad")
                    }
                ...
                func updateNotesCountLabel() {
                    ...
                    _ = locationNameLabel.text ?? "Unknown Location"
